...
//       // ✅ Limpiar el store DESPUÉS del signOut exitoso
//       clearUser()
...
// components/nav-user.tsx (Actualizado)
export const NavUser = React.memo(function NavUser({ open, user }: { open: boolean, user: any }) {
  ...
  // ✅ Estados de loading
  const isSigningOut = isLoading('signingOut')
  // const isFetchingUser = isLoading('fetchingUser')
  ...
  const handleSignOut = useCallback(async () => {
    ...
    try {
      ...
      await authClient.signOut({
        fetchOptions: {
          onSuccess: async () => {
            clearUser() // ✅ Limpiar inmediatamente
            router.push("/")
            await LoggingService.logActionWithErrorHandling(
              {
                userId: user.id,
  ...
  // ✅ Mostrar skeleton si hay sesión pero aún no hay datos del usuario
  // if (user && !userDb) {
  //   return <NavUserSkeletonClient open={open} />
  // }
