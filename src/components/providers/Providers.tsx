...
export function Providers({ children, initialSession }: Props) {
    ...
    useEffect(() => {
        ...
        // ✅ Si hay datos en store pero NO hay sesión válida = sesión expiró
        if (!sessionExists && hasUserInStore && !isLoggingOut) {
            clearUser()
    ...
    }, [initialSession?.user?.id, !!user, isLoading('signingOut'), isLoading('authRedirect'), clearUser, setUser, stopLoading, router])
